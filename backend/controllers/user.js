            ...
            bcrypt.compare(req.body.password, user.password)
                .then(valid => {
                    ...
                    res.status(200).json({
                        ...
                        console.log('Utilisateur connecter id n°: ' + user.id)
                        )
...
exports.editUser = (req, res) => {  
        ...
        User.findOne({where: {id:req.params.id}})
            .then(user => {
                if(user.profile !== "defaultUserProfile.png") { 
                    // Vérrifier si sa photo de profile
                    // n'est pas celle par défaut on peut la supprimer
                    fs.unlink(`images/${user.profile}`, (error) => {
                        if (error) throw err
                    })    
                } else {
                    console.log("impossible d'effacé l'image par défaut")
                }
            })
            .catch(error => res.status(400).json(error));
    }
    if (req.body.password) { // <- si le password a été modifié on enregistre le hash
        ...
        User.update(req.body, {where: {id: req.params.id}})
            .then(() => res.status(201).json({message: "Le profil à bien été actualisé"}))
            .catch(error => res.status(400).json(error));
    };
};
